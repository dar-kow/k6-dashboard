...
export const TestResultTabs: React.FC<TestResultTabsProps> = memo(({
    files,
    selectedFile,
    onFileChange,
}) => {
                ...
                <nav className="flex space-x-2 overflow-x-auto p-2">
                    {files.length === 0 ? (
                        ...
                        files.map((file) => (
                            <button
                                key={file.name}
                                className={`px-4 py-2 font-medium rounded-md transition-colors whitespace-nowrap ${selectedFile === file.name
...
TestResultTabs.displayName = 'TestResultTabs';
