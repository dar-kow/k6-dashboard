...
import { TestDirectory } from '../../../types';
...
export const TestRunComparison: React.FC<TestRunComparisonProps> = memo(({
    directories,
    currentRun,
    onCompareWith,
}) => {
    ...
    }, [compareRun, onCompareWith]);

    const handleQuickCompare = useCallback((dirName: string) => {
        onCompareWith(dirName);
    }, [onCompareWith]);
                ...
                <div className="mt-4 space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2 mt-3">
                        ...
                        {availableForComparison.slice(0, 3).map((dir) => (
                            <button
                                key={dir.name}
                                onClick={() => handleQuickCompare(dir.name)}
...
TestRunComparison.displayName = 'TestRunComparison';
